  ...
  const handleScan = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setScanButton(true); // Gagal scan
        setIsLoading(false); // Hentikan loading
      } else {
        console.log('Scan command sent successfully');
        setScanButton(false); // Ubah tombol menjadi "Stop"
        setIsLoading(true); // Hentikan loading karena scan dimulai
      }
    } catch (error) {
      console.error('Error sending scan command:', error);
      setScanButton(true); // Reset tombol
      setIsLoading(false); // Hentikan loading
    }
  };
  ...
  const handleStopScan = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setScanButton(true); // Ubah tombol menjadi "Start"
      setTimeout(() => {
        setReceiving([]);
      }, 500);
      }
